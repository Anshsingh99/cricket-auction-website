    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      {teams.map((team) => (
        ...
        >
          ...
          <p className="mb-2text-black text-black">
            Remaining Budget: {team.budget} points
          </p>
...
export function AuctionPage() {
  ...
  const handleStartAuction = () => {
    ...
    const interval = setInterval(() => {
      ...
      dispatch({
        type: "SELECT_PLAYER",
        player: availablePlayers[randomIndex],
      });
  ...
  useEffect(() => {
    if (availablePlayers.length === 0 && unsoldPlayers.length > 0) {
      const startNewRound = window.confirm(
              ...
              <div className="bg-black/50 rounded-xl p-8 shadow-lg mt-8 border border-red-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {unsoldPlayers.map((player) => (
                    ...
                    >
                      {player.name}{" "}
                      <span className="text-red-300">({player.type})</span>
                    </div>
                  ))}
                </div>
              </div>
